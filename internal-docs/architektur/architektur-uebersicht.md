...
| Benutzerrolle → Administrationsfunktionen | Benutzer-, Blocklist- und Audit-Verwaltung | `/admin`, Benutzer-API, Blocklist- und Audit-Endpunkte prüfen die Session-E-Mail gegen `SUPER_ADMINS` über `lib/admin-auth.ts`; Admin-Aktionen werden in `audit_logs` mit Ressourcentyp `admin` protokolliert. |
...
| Blocklist-Statistik und Blocklist-Update waren zuvor ohne Super-Admin-Schutz erreichbar. | Informationspreisgabe und ressourcenintensive Aktualisierung durch nicht autorisierte Aufrufer. | Hoch (geschlossen) | GET/POST `/api/admin/blocklist` erzwingen `requireSuperAdmin`; Refresh schreibt Audit-Eintrag. | API-Autorisierungstests für anonym, Benutzer und Super-Admin. | `app/api/admin/blocklist/route.ts`, `lib/admin-auth.ts`, vier Regressionstests. |
