...
| Benutzerrolle → Administrationsfunktionen | Benutzer-, Blocklist-, Audit- und Übersichtsverwaltung | `/admin`, `/admin/audit`, `/api/admin/overview`, `/api/admin/audit-logs` und Benutzer-API prüfen Super-Admin über `requireSuperAdminApiAccess` in `lib/admin-auth.ts`; Blocklist-GET/POST, Löschungen und Refreshs werden in `audit_logs` mit Ressourcentyp `admin` protokolliert; Admin-APIs sind über `admin_api` (120 Anfragen je Super-Admin und IP in 15 Minuten, fail-closed) begrenzt. |
