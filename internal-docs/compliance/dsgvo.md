...
| Security, Rate Limit, Blocklist | Besucher/Nutzer; IP oder User-ID, Aktion, Zeit, URL bei Ereignis | Verfuegbarkeit, Missbrauchs- und Phishingschutz | Art. 6 Abs. 1 lit. f, dokumentierte Interessenabwaegung | DB, Plattformlogs, optional Google Safe Browsing | Passkey-Starts nutzen 10 Anfragen je IP/5 Minuten; Paste-Unlocks 5 je IP/Paste/15 Minuten. DB-Rate-Limit-Zeilen werden nur anlassbezogen fuer denselben Identifier bereinigt, und der Dienst arbeitet bei DB-Fehler fail open. Logfrist nicht belegt. |
...
- `app/protected/paste/[slug]/page.tsx`, `app/api/pastes/[slug]/unlock/route.ts`, `app/p/[slug]/page.tsx`, `app/p/[slug]/raw/route.ts`, `lib/paste-access.ts`: Passwort nur im POST-Body, bcrypt-Pruefung, gebundener HttpOnly-Nachweis sowie Haupt-/Raw-/Ablaufkontrolle; 15 Regressionstests.
...
| Paste-Passwortschutz oder Zugriffsnachweis regrediert; produktive Cookie-/Secret-Eigenschaften sind unbestaetigt | Unbefugte Inhaltsweitergabe | Niedrig nach Codefix, betrieblich unbestaetigt | POST-basierten Hashvergleich und an Slug/Hash/Ablauf gebundenen HttpOnly-Nachweis in Haupt-/Raw-Ansicht erhalten | 15 Regressionstests plus produktiver Browser-/Cookie-/Proxy-Log-Test | Paste-Routen, `lib/paste-access.ts`, Commit `44daa90` |
...
| 21.07.2026 | Compliance-Dokumentation | Paste-VVT, Rate-Limit, Nachweise und Risiken auf POST-basierten bcrypt-Nachweis, gebundenen HttpOnly-Cookie und 15 Regressionstests aktualisiert; produktive Cookie-/Proxy-Evidenz bleibt offen | Daily Evolution bis Commit `44daa90` |
