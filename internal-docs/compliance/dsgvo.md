...
## Einleitung
Dieses Dokument verbindet das interne Verzeichnis von Verarbeitungstaetigkeiten (VVT), die Datenschutzkontrollen und die nach Repository-Stand erkennbare Lueckenanalyse fuer Zhort. Es basiert auf dem Code- und Datenbankschema vom 20.07.2026, Commit `8611a8d`.
...
| Pastebin | Ersteller/Betroffene im Inhalt; Paste-Inhalt, Sprache, Owner-ID, Schutz-/Ablaufdaten | Inhaltsfreigabe | Art. 6 Abs. 1 lit. b; anonym lit. f nach Interessenabwaegung | Oeffentlichkeit bei oeffentlichen Pastes, Hosting/DB | Optionales Ablaufdatum sperrt Haupt- und Raw-Zugriff; kein Purge belegt. Geschuetzte Pastes nutzen POST-basierten bcrypt-Vergleich und einen maximal einstündigen, an Slug/Hash/Ablauf gebundenen HttpOnly-Nachweis; Passwortwerte erscheinen nicht mehr in Paste-URLs. |
...
| Security, Rate Limit, Blocklist | Besucher/Nutzer; IP oder User-ID, Aktion, Zeit, URL bei Ereignis | Verfuegbarkeit, Missbrauchs- und Phishingschutz | Art. 6 Abs. 1 lit. f, dokumentierte Interessenabwaegung | DB, Plattformlogs, optional Google Safe Browsing | Passkey-Starts nutzen 10 Anfragen je IP/5 Minuten; Paste-Unlocks 5 je IP/Paste/15 Minuten und serialisieren denselben Schluessel transaktional. Paste-Unlocks arbeiten bei DB-Fehler fail-closed, andere Aktionen fail-open. Rate-Limit-Zeilen werden nur anlassbezogen bereinigt; Logfrist nicht belegt. |
...
|---|---|---|
| Passkey-Auth-Attempt nach erfolgreicher WebAuthn-Pruefung | Token-Hash 2 Minuten gueltig; NextAuth loescht die passende Zeile atomar bei erfolgreichem Verbrauch; jeder neue Start purgt abgelaufene Token-Attempts | Zusaetzlich einen unabhaengigen zeitgesteuerten Purge belegen, damit Loeschung nicht von einem weiteren Loginstart abhaengt; Verbrauch protokollieren ohne Token. |
| Passkey-Auth-Attempt vor WebAuthn-Pruefung | Eigene opak adressierte Zeile je Ceremony; Challenge 5 Minuten gueltig, wird vor dem Nachweis nur gelesen und beim atomaren Abschluss entfernt; jeder neue Start purgt abgelaufene Challenge-Attempts | Zusaetzlich einen unabhaengigen zeitgesteuerten Purge belegen. |
...
| Belastbarkeit und Wirksamkeitspruefung | 12 Passkey-Tests in 3 Dateien einschließlich Startbereinigung und NextAuth-Credentials-Grenze erfolgreich | Keine reale PostgreSQL-Konkurrenz-/WebAuthn-E2E-Pruefung; CI erzwingt Tests nicht; Datenschutz-, Restore-, Rechte- und Zugriffstests regelmaessig ausfuehren. |
...
- `lib/auth/passkey-auth-attempt.ts`, `drizzle/0004_secure_passkey_auth.sql`, `drizzle/meta/_journal.json`: opake Attempt-IDs, Laufzeiten, Purge bei jedem Start, atomarer Abschluss/Verbrauch, Ablaufindizes und journalisierte Tabelle; `drizzle/meta/0004_snapshot.json` fehlt.
- `app/s/[shortCode]/route.ts`, `lib/analytics.ts`: Klicktracking und Uebermittlung an ip-api.com.
- `app/protected/paste/[slug]/page.tsx`, `app/api/pastes/[slug]/unlock/route.ts`, `app/p/[slug]/page.tsx`, `app/p/[slug]/raw/route.ts`, `lib/paste-access.ts`, `lib/rate-limit.ts`: Passwort nur im POST-Body, bcrypt-Pruefung, gebundener HttpOnly-Nachweis, Haupt-/Raw-/Ablaufkontrolle sowie transaktional serialisiertes Fail-closed-Limit; 19 Regressionstests.
...
| Paste-Passwortschutz, Zugriffsnachweis oder Versuchslimit regrediert; produktive Cookie-/Secret-/Live-DB-Eigenschaften sind unbestaetigt | Unbefugte Inhaltsweitergabe | Niedrig nach Codefix, betrieblich unbestaetigt | POST-basierten Hashvergleich, gebundenen HttpOnly-Nachweis, Advisory-Lock und Fail-closed-Speicherfehler erhalten | 19 Regressionstests plus produktiver Browser-/Cookie-/Proxy-Log-/PostgreSQL-Konkurrenztest | Paste-Routen, `lib/paste-access.ts`, `lib/rate-limit.ts`, Commit `9cfdeea` |
| Link-Passwoerter und Freigabe-Access-Keys werden als Queryparameter uebergeben | Offenlegung in Browserhistorie, Logs oder Telemetrie | Mittel; fuer Pastes technisch geschlossen | Geheimnis per POST pruefen und kurzlebiges, gebundenes Zugriffstoken verwenden | Test, dass keine Geheimnisse in URLs/Logs erscheinen | Paste-Flow geschlossen; Protected-Link- und Passwortfreigabe-Routen offen |
...
| Ablaufdaten werden nicht durchgaengig zeitgesteuert geloescht; `passkey_auth_attempts` werden nur beim naechsten Start gepurgt | Verzoegerte Loeschung und Verstoss gegen Speicherbegrenzung | Hoch | Zentralen Purge fuer alle ablaufbaren Tabellen implementieren und den vorhandenen Start-Purge beibehalten | Taeglicher Lauf, Alarm und monatliche Stichprobe | `lib/auth/passkey-auth-attempt.ts` loescht abgelaufene Attempts bei jedem Start; unabhaengiger allgemeiner Purge fehlt |
...
| 20.07.2026 | Compliance-Dokumentation | Passkey-Verarbeitung nach Reviewfixes auf dedizierte Attempt-Zeilen, damaligen Teststand und registrierte Migration aktualisiert; operative Migration und reale DB-/WebAuthn-Pruefung bleiben offen | Reviewfixes bis Commit `6618792` |
| 20.07.2026 | Compliance-Dokumentation | Auf 12 Tests, Startlimit 10/5 Minuten, Purge bei jedem Start, Ablaufindizes und aktuellen HEAD aktualisiert; unabhaengiger Purge, reale DB-/WebAuthn-/Staging-Pruefung und `0004_snapshot.json` bleiben offen | Finales Hardening bis Commit `8611a8d` |
| 21.07.2026 | Compliance-Dokumentation | Paste-VVT, Rate-Limit, Nachweise und Risiken auf POST-basierten bcrypt-Nachweis, gebundenen HttpOnly-Cookie, transaktionale Fail-closed-Policy und 19 Regressionstests aktualisiert; produktive Cookie-/Proxy-/Live-DB-Evidenz bleibt offen | Daily Evolution bis Commit `9cfdeea` |
