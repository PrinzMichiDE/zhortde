...
- 15 Paste-Regressionsfälle in vier Testdateien; vollständige Suite 27 Tests in sieben Dateien.
...
| Paste-Passwort- und Raw-Bypass | Die Hauptansicht akzeptierte jeden nichtleeren Querywert; die Raw-Route prüfte weder Passwort noch Ablauf und konnte vertrauliche Inhalte offenlegen. | Passwort nur noch per Unlock-POST; bcrypt-Prüfung, 5 Versuche je IP/Paste in 15 Minuten, HMAC-Nachweis an Slug/Hash/Ablauf, HttpOnly-/SameSite-/Secure-/Pfadattribute, identische Haupt-/Raw-Prüfung und HTTP 410 bei Ablauf. | Paste-Seiten/-Routen, `lib/paste-access.ts`, `lib/rate-limit.ts` |
| Paste-Regressionstests | Die Schwachstelle und kryptografische Bindung benötigten reproduzierbare Negativ- und Positivnachweise. | 15 Tests prüfen URL-Bypass, Raw-Verweigerung, Ablauf, gültigen Zugriff, falsches Passwort, Rate-Limit, Cookie-Attribute, Manipulation, Bindung und Tokenablauf; vollständige Suite 27/27 grün. | Vier neue/erweiterte Paste-Testdateien |
...
| `npm test` am 2026-07-21 | Bestanden: 7 Testdateien, 27 Tests, 0 Fehler | 15 neue Paste-Vertragsfälle und alle bestehenden Passkey-Fälle erfolgreich. |
...
| Paste-Regression und Fix | `83013a4`, `44daa90` | Reproduziert Raw-/Ablauf-Bypass und implementiert Unlock-POST, HMAC-Cookie, Rate-Limit, Haupt-/Raw-Kontrolle und 15 Tests. |
...
| Regressionstests | Passkey-Tests unter `lib/auth/` sowie vier Paste-Testdateien unter `app/api/pastes/`, `app/p/` und `lib/` | Sieben Dateien und 27 bestandene Tests. |
...
| Paste-Zugriffsnachweis wird umgangen, falsch gebunden oder in Produktion mit schwachem/fehlendem Secret betrieben | Vertraulichkeitsverlust oder nicht nutzbarer Unlock | Niedrig nach Codefix, betrieblich unbestätigt | bcrypt-POST, HMAC an Slug/Hash/Ablauf, HttpOnly/Pfad/TTL und Secret-Policy | 15 Regressionstests; Browser-/Cookie-/Proxy-Log-Test und Secret-Nachweis vor Release | `lib/paste-access.ts`, Paste-Routen; technischer Commit `44daa90` |
...
| 2026-07-21 | Technische Dokumentation / automatisierte Revision | Paste-Passwort- und Raw-/Ablauf-Bypass, POST-basierten bcrypt-Nachweis, gebundenen HttpOnly-Cookie, Rate-Limit und 15 neue Tests dokumentiert; produktiver Browser-/Proxy-Nachweis bleibt offen | Daily Evolution bis `44daa90` |
